/**
 * Private chat — admin thread messages.
 *   GET  /api/v1/admin/private/conversations/{id}/messages?after= → { messages }
 *   POST /api/v1/admin/private/conversations/{id}/messages { message, attachment? } → { message }
 * Mirrors the Laravel AdminPrivateChatController (attachments + typing clear).
 */
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const rows = after > 0
    ...
    : await env.blog_db
        .prepare(
          'SELECT id, sender_id, message, attachment, read_at, created_at FROM private_chat_messages WHERE session_id = ? ORDER BY id LIMIT 200',
        )
        .bind(id)
        .all<Record<string, unknown>>()
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const row = await env.blog_db
    .prepare('SELECT id, sender_id, message, attachment, read_at, created_at FROM private_chat_messages WHERE id = ?')
