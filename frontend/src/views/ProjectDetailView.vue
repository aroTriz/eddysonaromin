  ...
  <div class="mx-auto w-full max-w-3xl px-6 py-12 md:py-16">
    ...
    <AsyncState :loading="loading" :error="error" :on-retry="load">
      <template v-if="project">
        <header class="mt-6">
          <div class="flex flex-wrap items-center gap-2 font-mono text-[12.5px] text-gray-400">
        ...
        <dl v-if="project.role" class="mt-8 rounded-xl border border-gray-200 bg-white p-5">
          <dt class="font-mono text-[11.5px] uppercase tracking-wide text-gray-400">role</dt>
          <dd class="mt-1 text-[14px] text-ink">{{ project.role }}</dd>
        </dl>

        <section class="mt-8">
          <h2 class="font-mono text-[12.5px] uppercase tracking-wide text-gray-400">technologies</h2>
